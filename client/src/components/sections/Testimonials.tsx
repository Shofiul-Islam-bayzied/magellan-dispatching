    ...
    <section className="py-24 bg-gray-100 text-foreground border-y border-gray-200">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-4xl mx-auto mb-16 sm:mb-20">
          ...
          <h2 className="text-3xl sm:text-4xl md:text-6xl font-black mb-4 sm:mb-6 uppercase tracking-tighter leading-tight">
            Drivers Are Making <span className="text-primary block sm:inline mt-1 sm:mt-0">More Money</span>
          </h2>
          <p className="text-lg sm:text-xl text-gray-600 font-sans max-w-2xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8">
          {reviews.map((review, i) => (
            <div key={i} className="bg-white border-t-4 border-primary p-6 sm:p-8 shadow-xl relative group hover:-translate-y-1 sm:hover:-translate-y-2 transition-transform duration-300">
              <div className="flex text-primary mb-4 sm:mb-6">
                {[...Array(review.rating)].map((_, j) => (
                  <Star key={j} className="w-4 h-4 sm:w-5 sm:h-5 fill-current" />
                ))}
              </div>
              <p className="text-base sm:text-lg font-sans text-gray-700 mb-6 sm:mb-8 italic leading-relaxed">"{review.content}"</p>
              <div className="border-t border-gray-100 pt-4 sm:pt-6 mt-auto">
                <p className="font-black text-lg sm:text-xl uppercase tracking-wider text-[#0B3C5D]">{review.name}</p>
                <p className="text-xs sm:text-sm text-primary font-bold uppercase tracking-widest mt-1">{review.role}</p>
              </div>
            </div>
          ))}
        </div>
